...
```
kanban/
├── index.html         # 보드 레이아웃 마크업 + 인증 오버레이
├── style.css          # 스타일 (CSS 변수, 컴포넌트, 인증 UI)
├── auth.js            # Supabase 인증 로직 (v2.0 신규)
...
| 결정 | 이유 |
|---|---|
| Supabase Auth | 백엔드 없이 OAuth + 이메일 인증 지원, 무료 tier로 충분 |
| CDN 로드 (`@supabase/supabase-js@2`) | 빌드 도구 없이 정적 배포 호환 |
| localStorage 유지 (Supabase DB 미사용) | 구현 단순, DB 스키마/RLS 설정 불필요 |
| GitHub Pages | 별도 서버 없이 정적 파일 무료 배포 |

## v2.0 구현 계획

1. `auth.js` — Supabase 클라이언트, 인증 상태 관리, 로그인/로그아웃
2. `index.html` — 인증 오버레이 마크업, 헤더 사용자 정보, Supabase CDN 로드
3. `app.js` — `initApp()` 패턴 (DOMContentLoaded 제거), 사용자별 스토리지 키
4. `style.css` — 인증 UI 스타일, 헤더 우측 영역
5. 설계 문서 업데이트 (TRD, PRD, PLAN, TASKS)
6. git subtree split → 새 레포(`hyunhong93/kanban-board`) push
7. GitHub Pages 활성화
8. Supabase Redirect URL 등록
...
### v2.0 (완료)
- [x] 로그인하지 않으면 보드에 접근할 수 없다
- [x] Google / GitHub / 이메일로 로그인할 수 있다
- [x] 로그인 후 헤더에 사용자 정보가 표시된다
- [x] 로그아웃 후 로그인 화면으로 돌아간다
- [x] 새로고침 후 세션이 유지된다
- [x] GitHub Pages URL에서 앱이 정상 동작한다
